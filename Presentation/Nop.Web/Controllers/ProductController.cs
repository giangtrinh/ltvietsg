...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var cachedPictures = _cacheManager.Get(productPicturesCacheKey, () =>
            {
                ...
                foreach (var picture in pictures)
                {
                    pictureModels.Add(new PictureModel()
                    {
                        ...
                        MediumSizeImageUrl = _pictureService.GetPictureUrl(picture, _mediaSettings.ProductThumbMediumPictureSizeOnProductDetailsPage),
            ...
            #region Discounts
            var displayDiscount = _discountService.GetAllowedDiscounts(product, _workContext.CurrentCustomer);
            if (displayDiscount.Count > 0)
            {
                foreach (var item in displayDiscount)
                {
                    var discount = new DiscountModel();
                    PrepareDiscountModel(discount, item);
                    model.DiscountModels.Add(discount);
                }
            }
            #endregion
        ...
        #region Bestsellers
        public ActionResult BestSellers(ProductListPagingFilteringModel command)
        {
            ...
            if (command.PageSize <= 0)
            {
                command.PageSize = _catalogSettings.NumberOfBestsellersPageSize;
            }
            //load and cache report
            ...
            model.PagingFilteringContext.LoadPagedList(report);
            return View(model);
        }
        #endregion

        #region Sale
        public ActionResult Sale(ProductListPagingFilteringModel command)
        {
            ...
            if (command.PageSize <= 0)
            {
                command.PageSize = _catalogSettings.NumberOfSalePageSize;
            }
            var products = _productService.GetProductsSale(pageIndex: command.PageNumber - 1, 
                                                        pageSize: command.PageSize,
