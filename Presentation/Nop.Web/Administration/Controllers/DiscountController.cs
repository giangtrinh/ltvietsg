...
namespace Nop.Admin.Controllers
{
    public partial class DiscountController : BaseAdminController
    {
        ...
        public DiscountController(IDiscountService discountService, 
            ...
            IPermissionService permissionService, ILocalizedEntityService localizedEntityService,
        ...
        #region Utilities
        [NonAction]
        protected virtual void UpdateLocales(Discount discount, DiscountModel model)
        {
            foreach (var localized in model.Locales)
            {
                _localizedEntityService.SaveLocalizedValue(discount,
                                                               x => x.Name,
                                                               localized.Name,
                ...
                //search engine name
                var seName = discount.ValidateSeName(localized.SeName, localized.Name, false);
                _urlRecordService.SaveSlug(discount, seName, localized.LanguageId);
            }
        }
        [NonAction]
        ...
        public ActionResult Create(DiscountModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //search engine name
                model.SeName = discount.ValidateSeName(model.SeName, discount.Name, true);
                _urlRecordService.SaveSlug(discount, model.SeName, 0);
                //locales
                UpdateLocales(discount, model);
                //activity log
        ...
        public ActionResult Edit(int id)
        {
            ...
            {
                locale.Name = discount.GetLocalized(x => x.Name, languageId, false, false);
                locale.Description = discount.GetLocalized(x => x.Description, languageId, false, false);
                locale.MetaKeywords = discount.GetLocalized(x => x.MetaKeywords, languageId, false, false);
                locale.MetaDescription = discount.GetLocalized(x => x.MetaDescription, languageId, false, false);
                locale.MetaTitle = discount.GetLocalized(x => x.MetaTitle, languageId, false, false);
                locale.SeName = discount.GetSeName(languageId, false, false);
            });
